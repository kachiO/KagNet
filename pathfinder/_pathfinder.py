...
def find_paths(source, target, nhops=4, ifprint = False, **kwargs):
    try:
        global cpnet, concept2id, relation2id, id2relation, id2concept, cpnet_simple
        s = concept2id[source]
        t = concept2id[target]
    except:
        #KO: hack to load global variables
        if len(kwargs) == 6:
            concept2id = kwargs['concept2id']
            id2concept = kwargs['id2concept']
            relation2id = kwargs['relation2id']
            id2relation = kwargs['id2relation']
            cpnet_simple = kwargs['cpnet_simple']
            cpnet = kwargs['cpnet']
...
def process(filename, batch_id=-1, nhops=4):
    pf = []
